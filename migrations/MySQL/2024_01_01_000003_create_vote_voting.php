...
/**
 * MySQL migration: vote records (free and paid).
 * Depends on: vote_editions, vote_contestants
 */
return new class implements MigrationInterface
{
    public function up(SchemaBuilderInterface $schema): void
    {
        $schema->create('vote_voting', static function (Blueprint $t): void {
            ...
            $t->string('IPAddress', 45)->notNull()->comment('IPv4 or IPv6');
            $t->string('UserAgent', 500)->default('');
            $t->dateTime('CreatedAt')->default('CURRENT_TIMESTAMP');

            // Enforce one free vote per user+contestant+edition at DB level
